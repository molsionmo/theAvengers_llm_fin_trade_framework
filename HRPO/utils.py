...
def process_finance_sentiment_answer(pred: str) -> str:
    pred = pred.strip().lower()
    
    # 清理重复字符和噪音
    pred = re.sub(r'[#!@$%^&*()_+=\-\[\]{}|\\:";\'<>?,.]{3,}', '', pred)
    pred = re.sub(r'(.)\1{5,}', r'\1', pred)  # 移除连续重复字符
    pred = ' '.join(pred.split())  # 清理空白字符
    ...
    if "negative" in pred:
        ...
        # Try to find the last word that might be the sentiment
        words = pred.split()
        for word in reversed(words):
            clean_word = re.sub(r'[^a-zA-Z]', '', word).lower()
            if clean_word in ["negative", "positive", "neutral"]:
                return clean_word
        
        # 如果都没找到，尝试更宽松的匹配
        if any(x in pred for x in ["neg", "bad", "down", "loss", "drop"]):
            return "negative"
        elif any(x in pred for x in ["pos", "good", "up", "gain", "rise"]):
            return "positive"
        else:
            return "neutral"  # 默认返回neutral而不是原始字符串
...
def process_finance_sentiment(batch):
    prompts = [[
        {"role": "system", "content": SYSTEM_PROMPT},
